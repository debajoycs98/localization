...
def jaccard_similarity_score(matrix1, matrix2):
    # Initialize variables for intersection and union count
    intersection = 0
    union = 0
    
    # Iterate through each corresponding element in the matrices
    for i in range(len(matrix1)):
        for j in range(len(matrix1[0])):
            # Ignore cells with value 0.5
            if matrix1[i][j] != 0.5 and matrix2[i][j] != 0.5:
                # Increment the union count
                union += 1
                # Increment the intersection count if both values are equal
                if matrix1[i][j] == matrix2[i][j]:
                    intersection += 1

    # Calculate Jaccard similarity score
    if union == 0:
        return 0  # If there are no elements to compare, return 0
    else:
...
def find_closest_matrix(vector1, matrix_list):
    ...
    for index, vector2 in enumerate(matrix_list):
        ...
        # if(c1==c2):b=1
        # else: b=0
        distance = -1*(jaccard_similarity_score(vector1.reshape((50,50)),vector2.reshape((50,50))))
        print(distance)
...
class CreateMap(Node):
    ...
    def update_environment(self):
        ...
        for i,value in enumerate(self.temp):
            if(value>=0.6): self.binary[i]=1
            elif(value<0.6 and value>0.3): self.binary[i]=0.5
            else: self.binary[i] = 0
...
#     for x in range(min_x, max_x + 1):
#         for y in range(min_y, max_y + 1):
#             # Check if any corner of the cell is within the circle's radius
#             if (math.sqrt((x - cx) ** 2 + (y - cy) ** 2) <= radius or
#                 math.sqrt((x + 1 - cx) ** 2 + (y - cy) ** 2) <= radius or
#                 math.sqrt((x - cx) ** 2 + (y + 1 - cy) ** 2) <= radius or
#                 math.sqrt((x + 1 - cx) ** 2 + (y + 1 - cy) ** 2) <= radius):
#                 cells.add((x, y))
...
#     # Iterate over the list of matrices with their indices
#     for index, vector2 in enumerate(matrix_list):
#         distance = jaccard_score(vector1,vector2)*100
#         dist.append(distance)
...
#             min_index = index
#     dist = softmax(dist)*100 
#     dist = [round(a,0) for a in dist]
#     print(dist)
#     return min_index
...
#                 elif(value==0.5): self.binary[i] = 0.5
#                 else: self.binary[i]= 0
